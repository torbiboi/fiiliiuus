...
async def on_message(message):
    ...
    if msg.startswith("f!binary counting channel"):
        ...
        await message.channel.send(
            "Du kannst anfangen, hier in Binär zu zählen! `f!remove counting channel` beendet den Zählchannel. Die Nächste Zahl ist: 0")
    ...
    if message.channel.id in countingChannels:
        ...
        if answer == solutionBin and message.author.id != prev:  # wenn nicht zweimal der gleiche Nutzer gezählt hat
            with open("binarycount/" + str(message.channel.id), "w", encoding="utf-8") as hfile:
                hfile.write(str(int(str(solutionBin), 2) + 1) + "," + str(
                    message.author.id))  # Fortschritt und letzten Nutzer speichern
            await message.add_reaction("✅")
        else:
            with open("binarycount/" + str(message.channel.id), "w", encoding="utf-8") as hfile:
                hfile.write("0,0")
            await message.channel.send("Haha, <@" + str(message.author.id) + "> ruined the streak of " + str(
                solutionBin) + ". The next number is 0.")

    if msg.startswith("f!calc"):
        if msg == "f!calc stop":
            if (message.channel.id in list(
        ...
        if len(msg) > 4:
            await message.channel.send(
                "Falscher Syntax. `f!calc [type: +,-,*,:] [difficulty: 1-6; 0 for random] [number of rounds]`")
            return
    ...
    if msg == "f!help":
        embed = discord.Embed(title="ÜBERSICHT", description="Die meisten Commands:",
                              color=0xc853e6)  # erstellt so einen hübschen Kasten
    ...
    if any(word in msg.lower() for word in
           ["pneumonoultramicroscopicsilicovolcanoconiosis"]):  # kleines Easteregg oder so
        await message.add_reaction("🫶")
